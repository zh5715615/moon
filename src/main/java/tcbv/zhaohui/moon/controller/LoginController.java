...
@RestController
@RequestMapping("/v1/moon/login")
@Slf4j
public class LoginController {
    @PostMapping("walletLogin")
    @ApiOperation("钱包登录")
    public Rsp walletLogin(@RequestBody WalletLoginDto loginDto) {
        ...
        String token = ""; //TODO 这个地方需要给个token存到user表，为后续操作提供token
        //TODO 下面还要添加一个逻辑，user表有则更新token，无则插入新数据
        return Rsp.okData(ImmutableMap.of("address", loginDto.getAddress(), "token", token));
    }
}
